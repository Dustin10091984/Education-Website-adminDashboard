...
import { editTemplatesQuestion } from '../../features/templatesQuestions/templatesQuestionsSlice'

const EditTemplateQuestionsForm = () => {
  ...
  const handleFileImageChange = async (e) => {
    ...
    if (file) {
      ...
      try {
        ...
        toast.success('عکس یا ویديو سوال شما با موفقیت آپلود شد')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      dispatch(editTemplatesQuestion({ id, updatedData }))
        .unwrap()
        .then(() => {
          toast.success('سوال با موفقیت بروز رسانی شد', {
            onClose: () => {
              setTimeout(() => {
                navigate('/templates-questions')
              }, 2000)
            },
            autoClose: 2000,
          })
...
export default EditTemplateQuestionsForm
