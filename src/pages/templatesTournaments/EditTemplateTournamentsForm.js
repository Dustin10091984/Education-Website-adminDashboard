...
import { editTemplatesTournament } from '../../features/templatesTournaments/templatesTournamentsSlice'

const EditTemplateTournamentsForm = () => {
  ...
  const handleFileImageChange = async (e) => {
    ...
    if (file) {
      ...
      try {
        ...
        toast.success('عکس یا ویديو تورنومنت شما با موفقیت آپلود شد')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      dispatch(editTemplatesTournament({ id, updatedData }))
        .unwrap()
        .then(() => {
          toast.success('تورنومنت با موفقیت بروز رسانی شد', {
            onClose: () => {
              setTimeout(() => {
                navigate('/templates-tournaments')
              }, 1000)
            },
            autoClose: 1000,
          })
...
export default EditTemplateTournamentsForm
